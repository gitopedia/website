...
async function getIndexPage(slugParts) {
  ...
  try {
    ...
    let contentHtml = stripMdExtension(processed.toString());
...
async function getArticle(slugParts) {
  ...
  // e.g., ['science', 'physics', 'quantum-mechanics', 'dark-energy'] -> ['science', 'physics', 'quantum-mechanics']
  const dirSlugParts = slugParts.slice(0, -1);
  ...
  try {
    ...
    const processed = await processor.process(content);
    let contentHtml = stripMdExtension(processed.toString());
    contentHtml = makeImagePathsAbsolute(contentHtml, dirSlugParts);
    return {
      frontMatter: serializeFrontMatter(data),
      contentHtml
    };
