...
// Also converts PNG references to AVIF format
function makeImagePathsAbsolute(html, slugParts) {
  ...
  return html.replace(/src="(?!https?:\/\/)(?!\/)([^"]*)"/g, (match, relativePath) => {
    // Convert .png to .avif
    const avifPath = relativePath.replace(/\.png$/i, '.avif');
    return `src="${basePath}/${avifPath}"`;
  });
}

// Convert LaTeX-style math delimiters to dollar-sign format for remark-math
// The markdown files use \(...\) and \[...\] but remark-math expects $...$ and $$...$$
function wrapEquations(content) {
  ...
  // 3. Also handle plain square brackets with LaTeX commands (legacy format)
  // [ equation ] on its own line -> $$ ... $$ (but not markdown links)
...
function getAllArticles() {
  ...
  return files.map((absPath) => {
    ...
    // Read frontmatter to get title, created date, model, and version
    let title = null;
    let created = null;
    let model = null;
    ...
    try {
      ...
      // Get article title from frontmatter (article field or title field)
      if (data.article) {
        title = String(data.article);
      } else if (data.title) {
...
// Get all domains (top-level directories with index.md)
function getAllDomains() {
  if (!fs.existsSync(COMPENDIUM_DIR)) return [];
  const entries = fs.readdirSync(COMPENDIUM_DIR, { withFileTypes: true });
  const domains = [];
  ...
  for (const e of entries) {
    ...
    const indexPath = path.join(COMPENDIUM_DIR, e.name, 'index.md');
    ...
    try {
      ...
      // Get header image path if exists
      const imgDir = path.join(COMPENDIUM_DIR, e.name, '_img');
      let headerImage = null;
      if (fs.existsSync(imgDir)) {
        // Look for domain_header-medium.avif or domain_header.avif
        const mediumImg = `${e.name}_header-medium.avif`;
        const fullImg = `${e.name}_header.avif`;
        if (fs.existsSync(path.join(imgDir, mediumImg))) {
          headerImage = `/${e.name}/_img/${mediumImg}`;
        } else if (fs.existsSync(path.join(imgDir, fullImg))) {
          headerImage = `/${e.name}/_img/${fullImg}`;
        }
      }
      ...
      domains.push({
        slug: e.name,
        href: `/${e.name}`,
        title: data.title || data.domain || e.name,
        headerImage
      });
    } catch (err) {
      // Ignore errors reading frontmatter
